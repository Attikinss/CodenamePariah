...
public static class EnvironmentQuerySystem
{
    ...
    {
        return s_ResolvedQueries.Find(query => query.ID == request.ID /*string.CompareOrdinal(query.ID, request.ID) == 0*/);
    }
    ...
    public static bool RetrieveQuery(QueryRequest request, out Query query)
    {
        if (request != null)
        {
            if (!s_QueriesLocked)
            {
                ...
                {
                    query = s_ResolvedQueries.Find(q => q.ID == request.ID /*string.CompareOrdinal(q.ID, request.ID) == 0*/);
    ...
    */
    ...
    private static bool AddToResolvedQueries(Query query)
    {
        if (!s_QueriesLocked)
        {
            ...
            lock (s_ResolvedQueries)
            {
                if (s_ResolvedQueries.Find(q => q.ID == query.ID /*string.CompareOrdinal(q.ID, query.ID) == 0*/) == null)
